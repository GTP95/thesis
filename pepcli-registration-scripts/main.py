...
if __name__ == '__main__':
    ...
    pepcli_invocation = ['pepcli', '--oauth-token-secret', arguments.oauth_token_secret, '--oauth-token-group', 'Data Administrator', 'register', 'participant', '--force', '--first-name', arguments.first_name, '--last-name', arguments.last_name, '--middle-name', arguments.middle_name, '--date-of-birth', arguments.date_of_birth]
    debug(pepcli_invocation)
    result = run(pepcli_invocation, capture_output=True)
    output = result.stdout.decode('utf-8')
    debug('pepcli participant creation output: ' + output)
