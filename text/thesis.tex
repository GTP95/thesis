...
the previously presented idea work, but without a SAML identity provider? Here's the catch: PEP doesn't directly interact with the IdP!!! There's an Apache server running
Shibboleth Service Provider in-between PEP and the SAML IdP, then Shibboleth forwards the verified attribute to PEP. Here's the idea: write a middleware that works alongside
Shibboleth; the user discloses its own attribute to this component, which in turn sends the disclosed attribute to PEP's authentication server. From PEP's perspective, there is no
difference between Shibboleth and this middleware.  
...
\subsection{The middleware}
It is a web application exposing HTTP endpoints. It exposes the following three endpoints:

\begin{itemize}
	\item qr - starts an Yivi session to disclose the user's PEP ID and returns a QR code to perform the session
...
precautions to reduce the likelihood of mistakes. It is written in Rust \ref{rust_memory_safety} using the Rocket web framework \cite{rocket}.  The code is minimal and the
application is stateless. This makes it easier to reason about the application's security: since it is stateless, it is possible to consider the security of each endpoint in
...
possible combination in contacting the endpoints. Where possible, I used established libraries instead of writing my own code.
