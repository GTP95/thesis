...
\usepackage{minted} %listings doesn't work well with Bash
...
%Added for showing subsubsection in index and giving them numbers to properly reference them in the text
...
deck of data cards. \par \todo{Is this clear? Maybe specify that a cell is the intersection of a row with a column?}
...
\subsection{Authenticating study participants, but how?}\label{authenticationg_but_how} \todo{Implement feedback on page 20's right side}
...
a SAML identity provider compatible with YIVI. Unfortunately, I could not get any of the existing SAML IdPs work with YIVI\todo{maybe mention the other extendable IdPs,
https://gluu.org/ https://docs.jans.io/}, and implementing one from scratch was out-of-scope for this project.
...
using the same credential across different instances of PEP. This is further discussed in \todo{Add reference to the future work section}.
...
\begin{figure}[H]
		\vspace*{-100pt} %This fixes vertical placement: without it, there's some empty space at the top and the page number at the bottom ends up inside the image
...
\begin{figure}[H]
		\includegraphics[scale=0.1]{middleware_qr_endpoint.png}
		\vspace{-10pt}
		\caption{The "qr" endpoint}
		\vspace{10pt}

		\includegraphics[scale=0.1]{middleware_status_endpoint.png}
		\vspace{-10pt}
		\caption{The "status" endpoint}
		\vspace{10pt}

		\includegraphics[scale=0.1]{middleware_token_endpoint.png}
		\vspace{-10pt}
...
\section{Future work}
Relax, sit down and drink a coffee. Consider replacing the middleware with a proper authentication provider, maybe Jannsen or Gluu. Consider creating special user group
"participant", similarly to the "Data Administrator" and "Access Administrator" groups. This would be the basis to then automatize part of the configuration explained in
\ref{pep_configuration} to give a study participant's account the correct permissions. Evaluate the tradeoff between using UUIDs and just check that no account with such credential
already exists. If not using UUIDs, then add to pepcli switches to list existing users and groups: it helps in checking whether a user with a given name already exists or
not\todo{check if now pepcli gives out an error in case a user or group already exists}. Implement parent \& child login.
...
Administrator} groups. Then, the parts that can't be automatized this way could be scripted or added to pepcli's add participant command \ref{pepcli}.\todo{check command}.\par For
...
already in possesion of a PEP credential. Manual transcription is error prone, so I would suggest to use \todo{add name} checksum as this is already used to prevent transcription
errors for short pseudonyms \cite{pep-blueprint}. If instead there is no interest in letting study participants reuse their credential, the uuid library crate \todo{add ref} could
...
In section \todo{add ref} the login solution is presented. There, the way to link a user to the corresponding participant data boils down to creating a participant group with a
