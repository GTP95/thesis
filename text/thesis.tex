...
a SAML identity provider compatible with YIVI. Unfortunately, I could not get any of the existing SAML IdPs work with YIVI \todo{maybe mention the other extendable IdPs, https://gluu.org/ https://docs.jans.io/}, and implementing one from scratch is out-of-scope for this project.
...
was successful, it will obtain the value for the disclosed attribute and send it to PEP's authentication server (i.e., it will perform PEP's authentication\todo{Should I detail
		how PEP's authentication flow works?} flow needed for PEP to trust the information send by this component) to obtain an authentication token. Finally, it will forward this authentication token to the requester as a JSON string. \par
